...
import { HomeIcon, MessageCircle, MapPin } from "lucide-react";
    ...
    <div className="relative min-h-screen w-full text-white overflow-hidden">
      ...
      <main className="relative z-10">
        <section className="mx-auto max-w-6xl px-4 sm:px-6 py-10 md:py-14">
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-center max-w-2xl mx-auto">
          ...
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.05 }} className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {branches.map((b) => (
              <Link key={b.slug} href={`/karachi/${b.slug}`} className="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] p-5 hover:bg-white/[0.06] backdrop-blur transition">
          ...
          <div className="mt-10 flex flex-col items-center gap-2" id="agent">
            <Link href="https://wa.me/92XXXXXXXXXX?text=Hi%20Sana%20Sarah%20Salon%2C%20I%20want%20info%20for%20Karachi." target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 rounded-2xl bg-gradient-to-tr from-fuchsia-600 to-pink-500 px-6 py-3 text-sm sm:text-base font-semibold shadow-xl hover:scale-[1.02] active:scale-[0.98] transition">
              <MessageCircle />
              Talk to Agent
            </Link>
